...
LATEST_DIR="$ROOT_DIR/releases/latest"
PACKAGE_NAME="Codex用量-macOS-$ARCH_LABEL"
STAGING_ROOT="${CODEX_USAGE_RELEASE_STAGING_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/codex-usage-menu-release.XXXXXX")}"
PACKAGE_DIR="$STAGING_ROOT/$PACKAGE_NAME"
ZIP_PATH="$LATEST_DIR/$PACKAGE_NAME.zip"
DMG_PATH="$LATEST_DIR/$PACKAGE_NAME.dmg"

trap 'rm -rf "$STAGING_ROOT"' EXIT
mkdir -p "$LATEST_DIR" "$PACKAGE_DIR"
rm -f "$ZIP_PATH" "$DMG_PATH"

APP_SOURCE="$(CODEX_USAGE_ARCHS="$RELEASE_ARCHS" "$ROOT_DIR/scripts/build-macos-app.sh")"
ditto "$APP_SOURCE" "$PACKAGE_DIR/$APP_NAME.app"
