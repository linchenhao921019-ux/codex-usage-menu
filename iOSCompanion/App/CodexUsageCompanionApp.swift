...
struct ContentView: View {
                ...
                if let snapshot {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ContentUnavailableView(
                            "等待同步",
                            systemImage: "wifi",
                            description: Text("确认 iPhone 与 Mac 在同一网络，并保持菜单栏 App 运行。")
                        )
    ...
    private func sourceText(for result: CodexUsageLoadResult) -> String {
        ...
        if result.isCached {
            return "数据来自：\(sourceName)（缓存）"
        }
        return "数据来自：\(sourceName)"
    }
}
